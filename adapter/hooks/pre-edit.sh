#!/bin/bash
# pre-edit.sh — Claude Code PreToolUse Edit|Write adapter for dev-discipline.
...
INPUT=$(cat)
RESULT=$(printf '%s' "$INPUT" | bash "$ROOT/core/cmd/dev-discipline-run.sh" pre-edit-write 2>/dev/null) || true
